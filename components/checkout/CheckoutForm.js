...
import {
  ...
  shippingInfo,
  processTransaction
} from "./../../store/actions/shipActions";
...
class CheckoutForm extends Component {
  ...
  handleSubmit = () => {
    ...
    if (!AcceptError) {
      const shippingRegion = states.find(
        x => x.code === this.state.shipping_address.region_code
      );
      ...
      const billingRegion = states.find(
        x => x.code === this.state.billing_address.region_code
      );
      this.setState(prevState => ({
        billing_address: {
      ...
      if (
        this.state.shipping_address.firstname === "" ||
        this.state.shipping_address.lastname === "" ||
      ...
      shippingMethod(shippingMethodData).then(shippingMethodResponse => {
        ...
        const shippingInfoData = {
          addressInformation: {
            ...
            shipping_carrier_code:
              shippingMethodResponse.payload[0].carrier_code,
            shipping_method_code: shippingMethodResponse.payload[0].method_code
          }
        };
        shippingInfo(shippingInfoData).then(shippingInfoResponse => {
  ...
  handleSameOrDifferentAddressChange = event => {
    event.target.value === "same_address"
      ? this.setState({
          same_address: true
        })
      : this.setState({
    ...
    const obj = {
      ...
      paymentMethod: {
        ...
        additional_data: {
          ...
          opaqueDataValue: this.state.payment_token
        }
      },
      billing_address: this.state.billing_address
    };
  ...
  render() {
    ...
    if (new Date() > setHours(setMinutes(new Date(), 0), 20)) {
      this.state.exclDate = new Date();
    }

    //console.log(setHours(setMinutes(new Date(), 0), 20));
    ...
    return (
      <section className="checkout-area ptb-20">
        <div className="container">
              ...
              <div className="billing-details">
                ...
                <div className="row">
                  ...
                  <div className="col-lg-12 col-md-6">
                    ...
                    <div className="form-group">
                      <label>
                        Select Delivery Time <span className="required">*</span>
                      </label>
                      <DatePicker
                        minDate={new Date()}
                        ...
                        showTimeSelect
                        excludeDates={[this.state.exclDate]}
                    ...
                    <li>
                      <label>
                        <input
                          ...
                          onChange={this.handleSameOrDifferentAddressChange}
                        />
          ...
          <div className="row">
            {this.state.orderSummary.length === 0 && (
